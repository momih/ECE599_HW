...
def gaussian_noise(X, v):
    """ Apply gaussian noise to data in X, in other words a fraction v of elements of X
    (chosen at random) is forced to zero.
    ...
    n_samples = X.shape[0]
    random_indices = np.random.randint(0, n_samples, v) 

    for i in random_indices:
        X_noise[i] = random_noise(X_noise[i], mode='gaussian')
