...
class DenoisingAutoencoder(object):
    ...
    def __init__(self, model_name='dae', pickle_name='test', test_name='testname',
                 n_components=256, main_dir='dae/', 
                 enc_act_func='tanh', dec_act_func='none', 
                 loss_func='mean_squared', num_epochs=10, batch_size=10, 
                 dataset='mnist', xavier_init=1, opt='gradient_descent', 
                 learning_rate=0.01, momentum=0.5, corr_type='none',
        ...
        self.global_models_dir = 'models/'  # dir to save/restore models
        self.global_data_dir = 'data/'  # directory to store algorithm data
        self.global_summary_dir = 'logs/'  # directory to store tensorflow summaries
    ...
    def fit(self, train_set, pickle_dict, validation_set=None, restore_previous_model=False):
        ...
        with tf.Session() as self.tf_session:
            print "\n Initializing utilities and ops"
            self._initialize_tf_utilities_and_ops(restore_previous_model)
            
            print "\n Training model"
            self._train_model(train_set, validation_set, pickle_dict)
    ...
    def _initialize_tf_utilities_and_ops(self, restore_previous_model):
        ...
        if restore_previous_model:
            print "\n Restoring model from: " + str(self.model_path)
            self.tf_saver.restore(self.tf_session, self.model_path)

        self.tf_summary_writer = tf.summary.FileWriter(self.tf_summary_dir, self.tf_session.graph)

    def _train_model(self, train_set, validation_set, pickle_dict):
        ...
        pickle_dict[self.test_name] = val_err
        with open('pickled/' + self.pickle_name, 'wb') as f:
            pickle.dump(pickle_dict, f)         
            
    def _run_train_step(self, train_set, corruption_ratio, i):
        ...
        batches = [_ for _ in dae_utils.gen_batches(shuff, self.batch_size)]
    ...
    def _corrupt_input(self, data, v):
        ...
        if self.corr_type == 'masking':
            x_corrupted = dae_utils.masking_noise(data, v)

        elif self.corr_type == 'salt_and_pepper':
            x_corrupted = dae_utils.salt_and_pepper_noise(data, v)

        elif self.corr_type == 'gaussian':
            x_corrupted = dae_utils.gaussian_noise(data, v)
    ...
    def _create_variables(self, n_features):
        ...
        W_ = tf.Variable(dae_utils.xavier_init(n_features, self.n_components, self.xavier_init), name='W_enc')
        bh_ = tf.Variable(tf.zeros([self.n_components]), name='encoder_bias')
        bv_ = tf.Variable(tf.zeros([n_features]), name='decoder_bias')
    ...
    def load_model(self, comps, model_path, cifar_features=3072):
        ...
        self._build_model(cifar_features)
    ...
    def _create_global_directories(self):
        ...
        """ 
        Create the three global directories
        """
        
        for d in [self.global_models_dir, self.global_data_dir, 
                  self.global_summary_dir, 'pickled/']:
    ...
    def _create_data_directories(self):
        ...
        models_dir = self.global_models_dir + self.main_dir
        data_dir = self.global_data_dir + self.main_dir
        summary_dir = self.global_summary_dir + self.main_dir
    ...
    def get_weights_as_images(self, width, height, outdir='img/', max_images=10, model_path=None):
        ...
        with tf.Session() as self.tf_session:
            ...
            for p in perm:
                ...
                dae_utils.gen_image(enc_w, width, height, image_path)
