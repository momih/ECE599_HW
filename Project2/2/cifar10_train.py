...
class Train(object):
    ...
    def print_params(self):
        print '\n Name: ' + str(FLAGS.version)
        print '\n Report frequency: ' + str(FLAGS.report_freq)
        print '\n Train steps: ' + str(FLAGS.train_steps)
        print '\n Batch size: ' + str(FLAGS.train_batch_size)
        print '\n LR: ' + str(FLAGS.init_lr)
        print '\n Padding size: ' + str(FLAGS.padding_size)
        print '\n CKPT path: ' + str(FLAGS.ckpt_path)
    ...
    def train(self):
        ...
        if FLAGS.is_use_ckpt is True:
            ...
            print 'Restored from checkpoint...' + str(FLAGS.ckpt_path)
        else:
            sess.run(init)
        ...
        for step in xrange(FLAGS.train_steps+1):
            ...
            if step % FLAGS.report_freq == 0:
                ...
                if step % FLAGS.train_steps == 0:
                    print "testing on full set"
                ...
                val_loss_list.append(validation_loss_value)
            ...
            if step % 2500 == 0 or (step + 1) == FLAGS.train_steps:
                checkpoint_path = os.path.join(train_dir, 'model.ckpt')
                saver.save(sess, checkpoint_path, global_step=step)

                df = {'step':step_list, 'train_error':train_error_list,
                                'validation_error': val_error_list,
                                'val_loss': val_loss_list,
                                'train_loss': train_loss_list }
                with open('error_files/errors_' + FLAGS.version, 'wb') as f:
                    pickle.dump(df, f)
